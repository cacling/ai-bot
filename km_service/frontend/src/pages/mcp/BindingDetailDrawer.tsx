/**
 * BindingDetailDrawer — 右侧抽屉，承载 Implementation 编辑功能
 *
 * 从 McpToolEditor 的 ImplStep + ApiPanel 迁移而来。
 * Sections: Header / Adapter Setup / Connector / Config / Status
 */
...
import { Save, ExternalLink, Trash2, Plus } from 'lucide-react';
import { mcpApi, type ToolImplementation, type Connector, type McpHandler } from './api';

interface Props {
  toolId: string | null; // null = closed
  onClose: () => void;
  onSaved: () => void;
...
export function BindingDetailDrawer({ toolId, onClose, onSaved, onOpenContract }: Props) {
  ...
  // API proxy fields (parsed from config)
  ...
  const loadImpl = useCallback(async (id: string) => {
    setLoading(true);
    try {
      const [implData, toolData] = await Promise.all([
  ...
  return (
    <Sheet open={!!toolId} onOpenChange={open => { if (!open) onClose(); }}>
      <SheetContent className="w-[520px] sm:max-w-[520px] overflow-y-auto">
        <SheetHeader>
          <SheetTitle className="flex items-center gap-2 text-sm">
            <span className="font-mono">{toolName || 'Loading...'}</span>
            {adapterType && (
              <Badge variant="outline" className={`text-[9px] px-1.5 py-0 ${ADAPTER_COLORS[adapterType] ?? ''}`}>
                {ADAPTER_OPTIONS.find(o => o.value === adapterType)?.label ?? adapterType}
              </Badge>
            )}
            {onOpenContract && toolId && (
              <Button variant="ghost" size="sm" className="text-[11px] h-6 ml-auto gap-1" onClick={() => onOpenContract(toolId)}>
                <ExternalLink size={10} /> Open Contract
              </Button>
            )}
          </SheetTitle>
        </SheetHeader>
          ...
          <div className="space-y-5 pt-4">
            ...
            {adapterType === 'script' && (
              <section className="space-y-2">
                ...
                {scriptContent && (
                  <div className="mt-2">
                    <div className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-1">Source (read-only)</div>
                    <pre className="text-[11px] font-mono bg-muted p-3 rounded-lg overflow-auto max-h-48 whitespace-pre-wrap">{scriptContent}</pre>
                  </div>
                )}
              </section>
            )}
            ...
            {adapterType === 'api_proxy' && (
              <section className="space-y-3">
                ...
                <div>
                  ...
                  <Textarea value={apiHeaders} onChange={e => setApiHeaders(e.target.value)} className="text-[11px] font-mono h-16 resize-none" placeholder='{"Authorization": "Bearer ..."}' />
                </div>
                <div>
                  <label className="text-[11px] text-muted-foreground mb-0.5 block">Body Template <span className="opacity-50">(空=直接转发参数)</span></label>
                  <Textarea value={apiBodyTemplate} onChange={e => setApiBodyTemplate(e.target.value)} className="text-[11px] font-mono h-16 resize-none" placeholder='{"phone": "{phone}"}' />
                </div>
                <div>
                ...
                <div className="space-y-2">
                  ...
                  {apiErrorMappings.map((em, i) => (
                    <div key={i} className="flex gap-1.5 items-center">
                      <Input value={em.status} onChange={e => { const n = [...apiErrorMappings]; n[i] = { ...n[i], status: e.target.value }; setApiErrorMappings(n); }} placeholder="401" className="w-14 text-[11px] font-mono" />
                      <span className="text-[11px] text-muted-foreground">{'\u2192'}</span>
                      <Input value={em.error_code} onChange={e => { const n = [...apiErrorMappings]; n[i] = { ...n[i], error_code: e.target.value }; setApiErrorMappings(n); }} placeholder="error_code" className="w-24 text-[11px] font-mono" />
            ...
            {adapterType && adapterType !== 'api_proxy' && (
              <section className="space-y-2">
                ...
                <Textarea
                  ...
                  className="text-[11px] font-mono h-24 resize-none"
