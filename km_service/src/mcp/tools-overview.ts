...
import { db } from '../db';
import { mcpServers, mcpTools } from '../db';
import { eq } from 'drizzle-orm';
import { getToolToSkillsMap } from '../engine-stubs';
...
function loadRawToolMap(): Map<string, { raw: RawToolRecord; serverName: string; serverStatus: string; enabled: boolean; disabled: boolean; mocked: boolean }> {
  ...
  try {
    ...
    return map; // DB 不可用时返回空 map，上层会补充 builtin + missing
  }
  for (const server of servers) {
    ...
    for (const t of tools) {
      map.set(t.name, {
        ...
        serverStatus: (server as Record<string, unknown>).status as string ?? 'active',
