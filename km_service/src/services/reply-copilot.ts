...
import { db } from '../db';
import { kmAssets, kmAssetVersions } from '../db';
...
export async function buildReplyHints(params: BuildParams): Promise<ReplyHints | null> {
  ...
  try {
    ...
    const s = top.structured as Record<string, unknown>;

    logger.info('reply-copilot', 'hints_built', {
      ms: Date.now() - t0,
      scene: (s.scene as Record<string, unknown>)?.code,
      confidence: confidence.toFixed(2),
      candidateCount: withStructured.length,
    });

    const scene = s.scene as { code: string; label: string; risk: string } | undefined;

    return {
      scene: scene ?? { code: 'unknown', label: '未知', risk: 'low' },
      required_slots: (s.required_slots as string[]) ?? [],
      recommended_terms: (s.recommended_terms as string[]) ?? [],
      forbidden_terms: (s.forbidden_terms as string[]) ?? [],
      reply_options: (s.reply_options as Array<{ label: string; text: string }>) ?? [],
      next_actions: (s.next_actions as string[]) ?? [],
