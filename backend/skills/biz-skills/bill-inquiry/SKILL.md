---
name: bill-inquiry
description: 账单查询与费用解释技能，处理话费金额、费用明细解读、欠费原因、异常费用分析、发票引导；不直接处理增值业务退订（应转退订技能）
metadata:
  version: "3.0.0"
...
- 增值业务退订操作 → 转 `service-cancel`
- 主套餐变更/升降档 → 转 `plan-inquiry`
- App 技术故障（闪退、登录、安装） → 转 `telecom-app`

### 高冲突场景澄清

当用户提到"不认识的扣费""多扣了钱，帮我退掉"时，先澄清意图：
> "您现在更想先弄清楚这笔费用是什么，还是想直接把对应业务退掉？"
- 如果用户想先了解费用来源 → 继续本技能
- 如果用户明确想退订 → 转 `service-cancel`

### 复合意图处理

对"怎么多了个会员扣费，帮我退掉"这类复合意图：先在本技能中解释费用来源，解释完毕后主动引导用户转至 `service-cancel` 完成退订。
