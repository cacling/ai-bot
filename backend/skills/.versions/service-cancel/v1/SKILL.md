...
stateDiagram-v2
    ...
    主套餐意图 --> 转套餐咨询: 只是想"每月少花点钱" → 引导至套餐查询（plan-inquiry） %% step:redirect-plan-inquiry %% kind:end %% guard:always
    引导前往营业厅 --> [*]
    ...
    state 标准退订流程 {
        ...
        列出剩余业务供选择 --> 用户选择下一个
        用户选择下一个 --> 说明退订影响: 用户选择下一个业务 %% guard:user.confirm
        用户选择下一个 --> 全部退订完成: 用户不再需要退订 %% guard:user.cancel
    }
    ...
    state 未知扣费流程 {
        ...
        反馈扣费结果 --> 反馈扣费完成: 说明退订已生效，当月费用按规则处理（当月不退，次月起不再扣费） %% step:unk-done %% kind:end
    }
...
- **每次只调用一次** `cancel_service`，等工具返回结果后再回复用户
- 回复完成后，若用户仍有其他待退订业务，列出剩余业务供用户选择，**等用户确认后**再调用下一次 `cancel_service`
- **禁止**在同一轮回复中连续调用 `cancel_service` 两次或以上
- 退订结果回复格式参考 `assets/cancel-result.md`
...
- 列出所有已订业务供用户选择，避免退错
- 退订成功后按 `assets/cancel-result.md` 模板回复（业务名、生效时间、剩余业务列表）
- 多个业务退订时，每完成一个就反馈结果并询问下一个，不要一次性输出所有结果
