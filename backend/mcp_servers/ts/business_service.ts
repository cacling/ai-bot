 ...
 * Port: 18004
 */
import { db, subscribers, subscriberSubscriptions, valueAddedServices, bills, mcpLog, startMcpHttpServer, eq, and, z, McpServer, performance } from "./shared.js";
...
function createServer(): McpServer {
  ...
  server.tool("cancel_service", "退订用户已订阅的增值业务（如流量包、短信包等）", {
    ...
    return { content: [{ type: "text", text: JSON.stringify({ success: true, phone, service_id, service_name: svc.name, monthly_fee: svc.monthly_fee, effective_end: svc.effective_end, message: `已成功退订「${svc.name}」，将于 ${svc.effective_end} 生效，本月费用正常收取` }) }] };
  });
