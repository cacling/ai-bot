/**
 * WorkOrderPreviewPane.tsx — Persistent detail preview panel (right side).
 */
...
interface WorkOrderPreviewPaneProps {
  lang: Lang;
...
export const WorkOrderPreviewPane = memo(function WorkOrderPreviewPane({
  lang,
  detail,
  loading,
}: WorkOrderPreviewPaneProps) {
  const t = T[lang];
...
function WorkItemContent({ data, lang }: { data: WorkItemDetail; lang: Lang }) {
  ...
  return (
    <>
      ...
      {data.childWorkOrders && data.childWorkOrders.length > 0 && (
        <div className="space-y-1">
          <h3 className="text-sm font-medium">{lang === 'zh' ? '子工单' : 'Child Work Orders'}</h3>
          <Separator />
          {data.childWorkOrders.map(wo => (
            <LabelValue key={wo.id} label={wo.status} value={wo.title} />
          ))}
        </div>
      )}
      ...
      {data.events && data.events.length > 0 && (
        <div className="space-y-1">
          ...
          <Separator />
          {data.events.map(e => (
            <LabelValue key={e.id} label={e.createdAt} value={`${e.eventType}${e.note ? ': ' + e.note : ''}`} />
          ))}
        </div>
      )}

      <div className="space-y-1">
        <h3 className="text-sm font-medium">{lang === 'zh' ? '时间' : 'Timestamps'}</h3>
        <Separator />
...
function IntakeContent({ data, lang }: { data: Intake; lang: Lang }) {
  ...
  return (
    <div className="space-y-1">
      ...
      <LabelValue label={t.wo_filter_status} value={<Badge variant={STATUS_VARIANT[data.status] ?? 'default'}>{data.status}</Badge>} />
      {data.riskScore != null && <LabelValue label={lang === 'zh' ? '风险分' : 'Risk'} value={data.riskScore} />}
      {data.confidenceScore != null && <LabelValue label={lang === 'zh' ? '置信度' : 'Confidence'} value={data.confidenceScore} />}
      {data.resolutionAction && <LabelValue label={lang === 'zh' ? '处置' : 'Action'} value={data.resolutionAction} />}
      <LabelValue label={t.wo_col_created} value={data.createdAt} />
    </div>
  );
}
  ...
  return (
    <>
      <div className="space-y-1">
        ...
        <LabelValue label={t.wo_filter_status} value={<Badge variant={STATUS_VARIANT[data.status] ?? 'default'}>{data.status}</Badge>} />
        {data.masterTicketId && <LabelValue label={lang === 'zh' ? '主工单' : 'Master'} value={data.masterTicketId} />}
        {data.latestItemId && <LabelValue label={lang === 'zh' ? '最新工单' : 'Latest'} value={data.latestItemId} />}
        <LabelValue label={t.wo_col_created} value={data.createdAt} />
        <LabelValue label={t.wo_col_updated} value={data.updatedAt} />
      </div>
      ...
      {data.intakes && data.intakes.length > 0 && (
        <div className="space-y-1">
          <h3 className="text-sm font-medium">{lang === 'zh' ? '关联线索' : 'Related Intakes'}</h3>
          <Separator />
          {data.intakes.map(i => (
            <LabelValue key={i.id} label={i.sourceKind} value={`${i.subject ?? '-'} (${i.status})`} />
          ))}
        </div>
      )}
    </>
