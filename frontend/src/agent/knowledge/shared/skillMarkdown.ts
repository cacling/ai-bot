...
function normalizeMermaidBlock(mermaid: string, newline: string): string {
  const trimmed = mermaid.trim();
  return `\`\`\`mermaid${newline}${trimmed}${newline}\`\`\``;
}

export function replaceCustomerGuidanceMermaid(skillMd: string, mermaid: string): string {
  const newline = skillMd.includes('\r\n') ? '\r\n' : '\n';
  const section = findCustomerGuidanceDiagramSection(skillMd);

  if (section.hasMermaidBlock && section.contentStart !== null && section.contentEnd !== null) {
    return `${skillMd.slice(0, section.contentStart)}${mermaid.trim()}${skillMd.slice(section.contentEnd)}`;
  }

  const block = normalizeMermaidBlock(mermaid, newline);

  if (section.hasSection && section.bodyStart !== null) {
    const insertion = `${newline}${newline}${block}`;
    return `${skillMd.slice(0, section.bodyStart)}${insertion}${skillMd.slice(section.bodyStart)}`;
  }

  const trimmed = skillMd.trimEnd();
  const spacer = trimmed.length === 0 ? '' : `${newline}${newline}`;
  return `${trimmed}${spacer}## ${CUSTOMER_GUIDANCE_HEADING}${newline}${newline}${block}${newline}`;
}
