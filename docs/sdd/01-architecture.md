...
flowchart TD
    ...
    BE[后端 Hono + Bun :18472<br/>WS /ws/chat → chat-ws.ts<br/>WS /ws/agent → agent-ws.ts<br/>WS /ws/voice → voice.ts<br/>POST /api/chat → agent/runner.ts<br/>❖ Vercel AI SDK generateText maxSteps=10<br/>❖ 会话历史：SQLite sessions / messages 表]:::whiteRed
...
| 分类 | 定义位置 | 谁能看到 | 用途 |
|------|---------|---------|------|
| **GLM 工具**（`VOICE_TOOLS`） | `routes/voice.ts` | GLM-Realtime 模型 | 用户可触发的业务操作 |
| **内部技能**（`handoff-analyzer.ts`） | `src/skills/` | 后端代码（仅转人工时调用） | 分析会话生成 Handoff Context，GLM 不可见 |
...
`voice-system-prompt.md` 和 `system-prompt.md` 均包含 `{{CURRENT_DATE}}` 占位符。`buildVoicePrompt()` 在构建系统提示词时将其替换为今日日期（zh-CN 格式，如 `2026年3月11日`），确保 Agent 的时间认知始终准确。
...
**后端拦截层**（`voice.ts` / `outbound.ts`，`ttsOverride = lang !== 'zh'` 时生效）：
...
**前端处理**（`VoiceChatPage.tsx` / `OutboundVoicePage.tsx`）：
...
| 文件 | 职责 |
|------|------|
| `backend/src/routes/voice.ts` | 入呼 TTS override 拦截 + 分句翻译 |
| `backend/src/routes/outbound.ts` | 外呼 TTS override（同上逻辑） |
| `backend/src/skills/tts.ts` | SiliconFlow CosyVoice2 TTS，支持 zh/en 双语音色 |
| `backend/src/skills/translate-lang.ts` | LLM 翻译服务 |
| `backend/src/agent/inbound-voice-system-prompt.md` | 日期处理规则 + 语言指令 |
| `frontend/src/pages/VoiceChatPage.tsx` | `tts_override` 前端处理 |
| `frontend/src/pages/OutboundVoicePage.tsx` | 同上 |
