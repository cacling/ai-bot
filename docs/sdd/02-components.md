...
## 5b. emotion-analyzer（backend/src/agent/card/emotion-analyzer.ts）
...
## 7b. 在线文字客服 WebSocket 路由（backend/src/chat/chat-ws.ts）
...
## 7c. 坐席工作台 WebSocket 路由（backend/src/agent/chat/agent-ws.ts）
...
## 7d. Session Bus（backend/src/services/session-bus.ts）
...
### 8.3 第二层：runner.ts 高亮函数（backend/src/engine/runner.ts）
...
## 9. 坐席卡片系统（frontend/src/agent/cards/）
...
## 11. 合规用语拦截引擎（backend/src/services/keyword-filter.ts）
...
## 14. 自然语言配置编辑（backend/src/agent/km/skills/skill-edit.ts）
...
## 15. 权限控制（backend/src/services/auth.ts）
...
## 17. 外呼语音系统（backend/src/chat/outbound.ts + frontend OutboundVoicePage.tsx）
...
## 18. useVoiceEngine 共享 Hook（frontend/src/chat/hooks/useVoiceEngine.ts）
...
## 19. TTS 语音合成（backend/src/services/tts.ts）
...
## 20. 实时翻译（backend/src/services/translate-lang.ts）
...
## 21. 语言会话管理（backend/src/services/lang-session.ts）
...
## 22. 跨窗口用户同步（frontend/src/chat/userSync.ts）
...
## 23. AI 技能创建器（backend/src/agent/km/skills/skill-creator.ts）
...
## 24. 灰度发布（backend/src/agent/km/skills/canary.ts）
...
## 25. 高风险变更审批（backend/src/agent/km/skills/change-requests.ts）
...
## 26. 回归测试用例（backend/src/agent/km/skills/test-cases.ts）
...
## 27. 知识管理模块（backend/src/agent/km/kms/）
