...
- 前端连接时，后端通过 `GlmRealtimeController` 建立到 GLM-Realtime 的 NodeWebSocket 连接
- 大多数事件（音频流、VAD 事件、字幕事件）直接双向透传
- `response.function_call_arguments.done` 由 controller 拦截处理（不透传）
- voice.ts 通过 `GlmSessionHooks` 注入特有行为：onGlmEvent（噪音静默、工具处理中拦截、告别门控、打断检测）、onBeforeToolCall（get_skill_instructions 短路）、sopCheck/sopRecord（SOP Guard）、mockToolCall（Mock 引擎）、onBotReply（合规检查 + 进度追踪）
...
## 4a. GlmRealtimeController（backend/src/services/glm-realtime-controller.ts）

### 4a.1 职责

统一封装 voice.ts 和 outbound.ts 共享的 GLM-Realtime 事件循环，消除 ~400 行重复代码。通过 `GlmSessionHooks` 接口注入 handler 特有行为。

### 4a.2 核心接口

```typescript
interface GlmSessionConfig {
  channel: 'voice' | 'outbound';
  sessionId: string; userPhone: string; lang: 'zh' | 'en';
  systemPrompt: string; tools: Array<Record<string, unknown>>;
  voiceName: string; vadInterruptResponse?: boolean;
}

interface GlmSessionHooks {
  onSessionReady?(ws, glmWs): Promise<void>;     // session.updated 后额外动作
  onBeforeToolCall?(ctx): Promise<string | null>; // 工具短路（voice: get_skill_instructions）
  enrichToolArgs?(toolName, args): void;          // 参数注入（outbound: task_id 等）
  sopCheck?(toolName): string | null;             // SOP Guard 拦截（voice）
  sopRecord?(toolName, result): void;             // SOP 记录
  mockToolCall?(name, args): { result, success } | null; // Mock 引擎（voice）
  getActiveSkillName?(): string | null;
  setActiveSkillName?(name): void;
  onBotReply?(transcript): void;                  // 回复后回调（合规/进度）
  onGlmEvent?(msg, ws): 'handled' | 'pass';      // 事件前置拦截
  onClose?(): void;                               // 连接关闭回调
}
```
...
function_call_arguments.done
  → transfer_to_human? → triggerHandoff
  → hooks.onBeforeToolCall? → 短路返回
  → hooks.enrichToolArgs? → 参数注入
  → preprocessToolCall
  → hooks.sopCheck? → 拦截
  → hooks.mockToolCall? → mock 返回
  → callMcpTool
  → hooks.sopRecord?
  → state.recordTool
  → inferSkillName → sendSkillDiagram
  → postprocessToolResult
  → GLM function_call_output + response.create
```
...
| 文件 | 职责 |
|------|------|
| `services/tts-override.ts` | 非中文 TTS 覆盖（翻译 + 生成目标语言语音） |
| `services/outbound-text-session.ts` | 文本模式外呼（Vercel AI SDK generateText） |
...
提供语音外呼能力，支持催收和营销两种场景。通过 `GlmRealtimeController` 与语音客服（voice.ts）共享 GLM 事件循环，使用独立的工具集和 system prompt。支持 `mode=text` 文本模式（跳过 GLM-Realtime，用 Vercel AI SDK `generateText` 驱动对话），用于 E2E 测试。
...
| `lang` | 语言 `zh` / `en` |
| `phone` | 客户手机号 |
| `mode` | `voice`（默认）或 `text`（文本模式，E2E 测试用） |
...
mode=voice（默认）：
  ...
  GLM 返回 session.updated
    ...
    └─ onSessionReady hook → 推送外呼技能流程图
  工具调用 → controller 统一管道（enrichToolArgs 注入 task_id 等）
  WebSocket 关闭 → 等待 pendingHandoff → 关闭 GLM 连接

mode=text（E2E 测试）：
  连接建立
    └─ OutboundTextSession.start() → generateText → 发送 bot 开场白
  收到 { type: 'chat_message', message } → handleMessage() → generateText with tools
  工具执行 → preprocessToolCall → callMcpTool → postprocessToolResult
  响应格式：{ source: 'bot', type: 'response', text, msg_id }
```
