...
**来源：** `backend/src/db/schema/`（Drizzle ORM，按职责拆分为两个模块）
...
| 模块 | 文件 | 表数 | 内容 |
|------|------|------|------|
| 电信业务 | `schema/business.ts` | 9 | plans, value_added_services, subscribers, subscriber_subscriptions, bills, mock_users, outbound_tasks, callback_tasks, device_contexts |
| 平台基础设施 | `schema/platform.ts` | 21 | sessions, messages, users, skill_registry, skill_versions, change_requests, test_cases, 13 张 km_* 表, mcp_servers |

统一导出：`schema/index.ts` 重新导出两个模块的全部表定义。
...
**来源：** `backend/src/db/schema/platform.ts` — `mcp_servers` 单表（工具和 mock 规则以 JSON 字段存储）

> **变更说明**：原 3 张表（mcp_servers + mcp_tools + mcp_mock_rules）已合并为 1 张表，工具定义、禁用列表和 mock 规则均以 JSON 字段内嵌。
...
export const mcpServers = sqliteTable('mcp_servers', {
  ...
  mock_rules:      text('mock_rules'),              // JSON: [{tool_name, match, response}]（原 mcp_mock_rules 表内容）
  // 时间戳
