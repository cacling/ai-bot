...
if [[ -f "$BASE_DIR/.env" ]]; then
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    [[ "$line" =~ ^(BACKEND_PORT|KM_SERVICE_PORT|MOCK_APIS_PORT|WORK_ORDER_PORT|MCP_INTERNAL_PORT|CDP_SERVICE_PORT|OUTBOUND_SERVICE_PORT|INTERACTION_PLATFORM_PORT|WFM_SERVICE_PORT|CHANNEL_HOST_PORT|BAILEYS_GATEWAY_PORT|FEISHU_GATEWAY_PORT|FRONTEND_PORT)= ]] && export "$line"
  done < "$BASE_DIR/.env"
fi
...
kill_port "$INTERACTION_PLATFORM_PORT" # interaction_platform
kill_port "$WFM_SERVICE_PORT"   # wfm_service
